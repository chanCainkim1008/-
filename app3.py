...
def search_players(query):
    results = hitters.find({'name': {'$regex': query, '$options': 'i'}})
    results = pitchers.find({'name': {'$regex': query, '$options': 'i'}})
    return results
...
def get_players():
    hitter_list = list(db.hitters.find({}, {'_id': False , 'name' : True}))
    pitcher_list = list(db.pitchers.find({}, {'_id': False , '이름' : True}))
    return jsonify({'hitter_list': hitter_list, 'pitcher_list': pitcher_list})
    

# @app.route("/player", methods=["GET"])
# def pitcher_get():
#     pitcher_list = list(db.pitchers.find({}, {'_id': False}))
